...
def configure_runner(destination, token, labels):
    logging.info(f"Configuring runner in {destination} with labels: {labels}.")
    os.chdir(destination)
    os.system(f"./config.sh --url https://github.com/{owner}/{repo} --token {token} --labels {labels}")
    logging.info("Runner configured successfully.")

def create_runners(owner, repo, token, runner_count, labels, runner_name):
    ...
    for i in range(runner_count):
        ...
        # Configure runner
        configure_runner(runner_dir, registration_token, labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--labels", required=True, help="Comma-separated labels for the runners.")
    ...
    create_runners(args.owner, args.repo, args.token, args.runner_count, args.labels, args.runner_name)
